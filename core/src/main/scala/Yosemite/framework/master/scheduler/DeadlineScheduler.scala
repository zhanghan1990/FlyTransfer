...
import Yosemite.framework.master.{CoflowInfo, CoflowState, FlowInfo}
...
class DeadlineScheduler extends OrderingBasedScheduler with Logging {
  ...
  val CONSIDER_DEADLINE = System.getProperty("Yosemite.master.consdierDeadline", "false").toBoolean
  val DEADLINE_PAD = System.getProperty("Yosemite.master.deadlinePadding", "0.1").toDouble
  val MIN_DEADLINE = System.getProperty("Yosemite.master.minDeadlineMillis", "200").toInt

  if (!CONSIDER_DEADLINE) {
    logError("Yosemite.master.consdierDeadline must be true for DeadlineScheduler")
    System.exit(1)
  }
