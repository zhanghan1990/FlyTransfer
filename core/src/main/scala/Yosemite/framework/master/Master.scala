...
import Yosemite.framework.master.scheduler._
import Yosemite.framework.master.ui.MasterWebUI
import Yosemite.util.{AkkaUtils, SlaveToBpsMap}
...
private[Yosemite] class Master(
  ...
  val NUM_MASTER_INSTANCES = System.getProperty("Yosemite.master.numInstances", "1").toInt
  val DATE_FORMAT = new SimpleDateFormat("yyyyMMddHHmmss") // For coflow IDs
  val SLAVE_TIMEOUT = System.getProperty("Yosemite.slave.timeout", "60").toLong * 1000

  val CONSIDER_DEADLINE = System.getProperty("Yosemite.master.considerDeadline", "false").toBoolean
  ...
  private[Yosemite] class MasterActor(
                                    ip: String,
                                    port: Int,
                                    webUiPort: Int)
    ...
    val masterPublicAddress = {
      val envVar = System.getenv("Yostemite_PUBLIC_DNS")
      if (envVar != null) envVar else ip
    }

    override def preStart() {
      logInfo("Starting Yosemite master at Yosemite://" + ip + ":" + port)
      // Listen for remote client disconnection events, since they don't go through Akka's watch()
      context.system.eventStream.subscribe(self, classOf[RemotingLifecycleEvent])
      if (!webUiStarted.getAndSet(true)) {
        println("start master ui")
        webUi.start()
      }
      ...
      case RegisterClient(clientName, host, commPort) => {
        ...
        if (hostToSlave.containsKey(host)) {
          ...
          currentSender ! RegisteredClient(
            client.id,
            slave.id,
            "Yosemite://" + slave.host + ":" + slave.port)

          logInfo("Registered client " + clientName + " with ID " + client.id + " in " +
            (now - st) + " milliseconds")
        } else {
          currentSender ! RegisterClientFailed("No Yosemite slave at " + host)
        }
      }
...
private[Yosemite] object Master {
  ...
  /**
    * Returns an `akka.tcp://...` URL for the Master actor given a YosemiteUrl `Yosemite://host:ip`.
    */
