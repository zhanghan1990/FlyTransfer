...
import Yosemite.ui.env.EnvironmentUI
import Yosemite.{Logging, Utils}
import org.eclipse.jetty.server.{Handler, Server}

/** Top level user interface for Varys */
private[Yosemite] class VarysUI() extends Logging {
  val host = Option(System.getenv("Yosemite_PUBLIC_DNS")).getOrElse(Utils.localHostName())
  val port = Option(System.getProperty("Yosemite.ui.port")).getOrElse(VarysUI.DEFAULT_PORT).toInt
  ...
  def bind() {
    try {
      ...
      logInfo("Started Yosemite Web UI at http://%s:%d".format(host, usedPort))
      server = Some(srv)
      boundPort = Some(usedPort)
    } catch {
      case e: Exception =>
        logError("Failed to create Yosemite JettyUtils", e)
        System.exit(1)
    }
  }
  ...
  private[Yosemite] def appUIAddress = host + ":" + boundPort.getOrElse("-1")

}

private[Yosemite] object VarysUI {
