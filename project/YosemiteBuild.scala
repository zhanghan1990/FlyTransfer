...
object YosemiteBuild extends Build {
  lazy val root = Project("root", file("."), settings = rootSettings) aggregate(Yosemite, demo)

  lazy val Yosemite = Project("Yosemite", file("Yosemite"), settings = YosemiteSettings)

  lazy val demo = Project("demo", file("demo"), settings = demoSettings) dependsOn (Yosemite)
  ...
  def YosemiteSettings = sharedSettings ++ Seq(
    name := "Yosemite-core",
    resolvers ++= Seq(
  ...
  def demoSettings = sharedSettings ++ Seq(
    name := "Yosemite_Demo"
  )
